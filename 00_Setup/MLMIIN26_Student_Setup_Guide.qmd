...
Make a `git pull` from the `origin` and `upstream` repositories to get the latest course files and keep in sync with your GitHub repo. Instructions follow.
:::
...
```
and press *Enter*. Wait for the operation to complete. Then 

```bash
git rebase upstream/main
```
And wait for the operation to complete. Finally, type 
```bash
...
#### 8.2.2.2 Sync with the upstream repo  (every session)
...
When the terminal opens in the bottom panel, make sure that you are located in the `MLMIINfrk` folder (it should appear in the prompt). Then type
